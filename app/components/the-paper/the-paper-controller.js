...
unipaper.controller('thePaperController', function($scope, contentful, environmentVariables, $rootScope, utilityFactory, $mdDialog, $window) {
  ...
  $scope.monthDropdownOptions = $scope.currentMonthAndYear = false;
  ...
  var fillMonthDropdownOptions = function() {
    ...
    while(year <= currentMonthAndYear.year) {
      ...
      for(i; i < 12; i++) {
        options.push($scope.monthLookup[i] + ' ' + year);
    ...
    $scope.currentMonthAndYear = $scope.monthAndYear = currentMonthAndYear.month + ' ' + currentMonthAndYear.year;
